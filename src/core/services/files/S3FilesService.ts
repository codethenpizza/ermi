import {CreateOptions, FilesService} from "@core/services/files/FilesService";
...
export class S3FilesService implements FilesService {
    ...
    async create(Body: Buffer, name: string, {mimeType, rewrite}: CreateOptions = {}): Promise<string> {
        name = S3FilesService.formatName(name);

        const ContentType = mimeType || S3FilesService.getContentType(name);
